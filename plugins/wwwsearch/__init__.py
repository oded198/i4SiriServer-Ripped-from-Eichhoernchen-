...
class wwwSearch(Plugin):
    ...
    @register("en-US", "(web search.*)|(web.*)|(google.*)|(google search.*)")
    @register("en-AU", "(web search.*)|(web.*)|(google.*)|(google search.*)")
    ...
    def webSearch(self, speech, language):
        if (language == "en-GB"):
            ...
            elif (speech.find('Google search') == 0):
                speech = speech.replace('Google search',' ',1)
            elif (speech.find('Google') == 0):
                speech = speech.replace('Google',' ',1)
            speech = speech.strip()
            if speech == "":
                speech = self.ask("what is your query?")
        if (language == "en-US" or language == "en-AU"):
            if (speech.find('Web search') == 0):
                speech = speech.replace('Web search', ' ',1)
            elif (speech.find('Web') == 0):
                speech = speech.replace('Web',' ',1)
