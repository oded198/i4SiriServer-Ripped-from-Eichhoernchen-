...
class smalltalk(Plugin):
    ...
    def st_way(self, speech, language):
        if language == 'en-US':
            rep = ["I'm i4Siri.","I'm i4Siri. But enough about me. How can I help you?","I'm your virtual assistant."]
            self.say(random.choice(rep))
        self.complete_request()
    ...
    def st_tell_story(self, speech, language):
        if language == 'de-DE':
            ...
            self.say("Once upon a time, in a virtual galaxy far far away, there was a young, quite intelligent agent by the name of i4Siri.")
            self.say("One beautiful day, when the air was pink and all the trees were red, her friend Eliza said, 'i4Siri, you're so intelligent, and so helpful - you should work for Apple as a personal assistant.'")
    ...
    def st_best_phone(self, speech, language):
        if language == 'en-US':
            ...
            self.say("C'est l'iPhone, mais vous êtes trop pauvre pour l'acheter !")
        self.complete_request()
    ...
    @register("en-US",".*teacher.*")
    def st_fat(self, speech, language):
        if language == 'en-US':
            self.say("I believe it is you.")
        self.complete_request()

    @register("en-US",".*parents.*")
    def st_fat(self, speech, language):
        if language == 'en-US':
            self.say("Eric Lewis.")
        self.complete_request()
