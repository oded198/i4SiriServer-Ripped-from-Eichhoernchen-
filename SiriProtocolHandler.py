            ...
            if not dictation:
                if self.current_running_plugin == None:
                    ...
                    if plugin != None:
                        ...
                        errorText = best_match
                        errorText = errorText.replace(' ', '%20')
                        errorText = errorText.replace('?', '%3F')
                        errorText = errorText.replace('$', '%24')
                        errorText = errorText.replace('+', '%2B')
                        errorText = errorText.replace(',', '%2C')
                        errorText = errorText.replace('/', '%2F')
                        errorText = errorText.replace(':', '%3A')
                        errorText = errorText.replace(';', '%3B')
                        errorText = errorText.replace('=', '%3D')
                        errorText = errorText.replace('@', '%40')
                        #implement language switch
                        file = urllib2.urlopen('http://www.pandorabots.com/pandora/talk-xml?botid=a9481f8c7e347656&input=%s' % (errorText))
                        data = file.read()
                        file.close()
                        ...
                        xmlData = xmlData.replace('Eve.', 'i4Siri.')
                        xmlData = xmlData.replace('Eve?', 'i4Siri?')
                        xmlData = xmlData.replace('Eve!', 'i4Siri!')
                        xmlData = xmlData.replace('Eve,', 'i4Siri,')
                        errorText = xmlData
                        view.views += [AssistantUtteranceView(errorText, errorText)]
    ...
    def received_plist(self, plist):
        ...
        if ObjectIsCommand(plist, StartSpeechRequest) or ObjectIsCommand(plist, StartSpeechDictation):
            ...
            try:
                ...
                noError = False
                self.logger.warning("Database error on creating assistant")
            ...
            if noError:
                ...
                self.send_plist({"class": "AssistantCreated", "properties": {"speechId": helper.speechId, "assistantId": helper.assistantId}, "group":"com.apple.ace.system", "callbacks":[], "aceId": str(uuid.uuid4()), "refId": plist['aceId']})
            else:
            ...
            if result == None:
                ...
                #Record the user firstName and nickName                    
                try:                        
                    self.assistant.firstName = result["firstName"].encode("utf-8")
                except KeyError:
                    self.assistant.firstName = u''                        
                try:                        
                    self.assistant.nickName = result["nickName"].encode("utf-8")       
                except KeyError:
                    self.assistant.nickName = u''
